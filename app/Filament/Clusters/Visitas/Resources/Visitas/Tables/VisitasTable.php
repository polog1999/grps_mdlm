...
class VisitasTable
{
    ...
    {
        return $table
        ->modifyQueryUsing(fn($query) => $query->leftjoin('personas', 'visitas.persona_id','=','personas.id')->select('visitas.*'))
        ->columns([
            TextColumn::make('personas.numero_documento')->label('Nombres y Apellidos')->searchable()->sortable(),
