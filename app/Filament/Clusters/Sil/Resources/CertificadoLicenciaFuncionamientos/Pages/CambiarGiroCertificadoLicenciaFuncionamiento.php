...
class CambiarGiroCertificadoLicenciaFuncionamiento extends EditRecord
{
    ...
    protected static ?string $title = 'Cambiar Giro Certificado de Licencia de Funcionamiento';
    ...
    protected function handleRecordUpdate(\Illuminate\Database\Eloquent\Model $record, array $data): \Illuminate\Database\Eloquent\Model
    {
        ...
        try {
            ...
            if (!empty($resultado) && isset($resultado[0])) {
                ...
                Log::error('No se recibió respuesta del servidor al cambiar giro licencia', ['resultado' => $resultado]);
                throw new \Exception('No se recibió respuesta del servidor');
            }

        } catch (\Exception $e) {
            Log::error('Excepción al cambiar giro licencia', [
                'mensaje' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            Notification::make()
                ->title('Error al cambiar giro licencia')
                ->body($e->getMessage())
                ->danger()
                ->send();
