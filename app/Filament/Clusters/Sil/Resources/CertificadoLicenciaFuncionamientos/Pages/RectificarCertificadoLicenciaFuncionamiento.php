...
class RectificarCertificadoLicenciaFuncionamiento extends EditRecord
{
    ...
    protected static ?string $title = 'Rectificar Certificado de Licencia de Funcionamiento';

    protected function getHeaderActions(): array
    {
        return [
            // No actions needed for rectify page
        ];
    }
    ...
    protected function handleRecordUpdate(\Illuminate\Database\Eloquent\Model $record, array $data): \Illuminate\Database\Eloquent\Model
    {
        ...
        try {
            ...
            Log::info('Iniciando proceso de rectificación de licencia', [
            ...
            if (!empty($resultado) && isset($resultado[0])) {
                ...
                if ($nuevoLicId > 0) {
                    // Actualizar el estado de la licencia original a RECTIFICADO (ID 13)
                    // El procedimiento almacenado spu_licencia_upd_duplicar2 lo pone como DUPLICADO por defecto
                    $record->update(['esl_id' => 13]);

                    Notification::make()
                        ->title('Licencia rectificada exitosamente')
                        ->body("Nueva licencia creada con ID: {$nuevoLicId}. La licencia original ha sido marcada como rectificada.")
                        ->success()
                        ->send();
                    ...
                    throw new \Exception($spResult->mensaje ?? 'Error desconocido al rectificar');
                }
            } else {
                Log::error('No se recibió respuesta del servidor al rectificar licencia', ['resultado' => $resultado]);
                throw new \Exception('No se recibió respuesta del servidor');
            }

        } catch (\Exception $e) {
            Log::error('Excepción al rectificar licencia', [
                'mensaje' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            Notification::make()
                ->title('Error al rectificar licencia')
                ->body($e->getMessage())
                ->danger()
                ->send();
