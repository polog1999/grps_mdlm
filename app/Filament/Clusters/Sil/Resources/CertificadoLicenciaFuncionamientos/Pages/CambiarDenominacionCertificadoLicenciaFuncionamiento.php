...
class CambiarDenominacionCertificadoLicenciaFuncionamiento extends EditRecord
{
    ...
    protected static ?string $title = 'Cambiar Denominación Certificado de Licencia de Funcionamiento';

    protected function getHeaderActions(): array
    {
        return [
            // No actions needed for change giro page
        ];
    }
    ...
    protected function handleRecordUpdate(\Illuminate\Database\Eloquent\Model $record, array $data): \Illuminate\Database\Eloquent\Model
    {
        ...
        try {
            ...
            Log::info('Iniciando proceso de cambio de giro de licencia', [
            ...
            if (!empty($resultado) && isset($resultado[0])) {
                ...
                if ($nuevoLicId > 0) {
                    // Actualizar el estado de la licencia original a CAMBIAR GIRO (ID 14)
                    // El procedimiento almacenado spu_licencia_upd_duplicar2 lo pone como DUPLICADO por defecto
                    $record->update(['esl_id' => 15]);

                    Notification::make()
                        ->title('Licencia cambiada de giro exitosamente')
                        ->body("Nueva licencia creada con ID: {$nuevoLicId}. La licencia original ha sido marcada como CAMBIADO DE GIRO.")
                        ->success()
                        ->send();
                    ...
                    throw new \Exception($spResult->mensaje ?? 'Error desconocido al cambiar giro');
                }
            } else {
                Log::error('No se recibió respuesta del servidor al cambiar denominación licencia', ['resultado' => $resultado]);
                throw new \Exception('No se recibió respuesta del servidor');
            }

        } catch (\Exception $e) {
            Log::error('Excepción al cambiar denominación licencia', [
                'mensaje' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            Notification::make()
                ->title('Error al cambiar denominación licencia')
                ->body($e->getMessage())
                ->danger()
                ->send();
