                    ...
                    Action::make('licencia-cambiar-giro')
                        ...
                        ->form([
                            \Filament\Forms\Components\Textarea::make('observacion')
                                ...
                                ->required()
                                ->rows(3)
                                ->placeholder('Ingrese el motivo por el cual desea cambiar de giro...'),
                        ])
                        ->action(function (CertificadoLicenciaFuncionamiento $record, array $data) {
                            try {
                                $user = auth()->user();
                                ...
                                if ($existe) {
                                    Notification::make()
                                        ->title('Solicitud pendiente')
                                        ->body('Ya existe una solicitud pendiente de cambio de giro para este registro.')
                                        ->warning()
                                        ->send();
                                    return;
                                }
                        ...
                        Action::make('licencia-cambiar-denominacion')
                        ->label('Cambiar Denominación licencia')
                        ->icon('heroicon-o-building-office')
                        ->tooltip('Cambiar Denominación licencia')
                        ->color('gray')
                        ->visible(fn() => auth()->user()->hasPermissionTo('change_denominacion::certificado_licencia_funcionamiento'))
                        ->url(function (CertificadoLicenciaFuncionamiento $record) {
                            $user = auth()->user();
                            $user_role_id = $user->modelHasRole?->role_id;
                            ...
                            $tienePermiso = ($user_role_id === 1 || $user_role_id === 2 || $user_role_id === 11) || \App\Models\SolicitudPermiso::query()
                                ...
                                ->where('user_id', $user->id)
                                ->where('estado', \App\Enums\SolicitudPermisoEstado::APROBADO)
                                ->where('tipo_accion', \App\Enums\SolicitudPermisoTipoAccion::CAMBIAR_DENOMINACION_LICENCIA)
                                ->exists();

                            return $tienePermiso ? CertificadoLicenciaFuncionamientoResource::getUrl('cambiar-denominacion', ['record' => $record]) : null;
                        })
                        ->modalHeading('Solicitar Permiso de Cambiar Denominación')
                        ->modalDescription('No tienes permisos directos para realizar esta acción. Por favor, indica el motivo para solicitar el cambio de denominación.')
                        ->modalSubmitActionLabel('Enviar Solicitud')
                        ->form([
                            \Filament\Forms\Components\Textarea::make('observacion')
                                ->label('Motivo de la solicitud')
                                ->required()
                                ->rows(3)
                                ->placeholder('Ingrese el motivo por el cual desea cambiar denominación...'),
                        ])
                        ->action(function (CertificadoLicenciaFuncionamiento $record, array $data) {
                            try {
                                $user = auth()->user();
                                ...
                                if ($existe) {
                                    Notification::make()
                                        ->title('Solicitud pendiente')
                                        ->body('Ya existe una solicitud pendiente de cambio de denominación para este registro.')
                                        ->warning()
                                        ->send();
                                    return;
                                }
                                ...
                                \App\Models\SolicitudPermiso::create([
                                    ...
                                    'record_id' => $record->lic_id,
                                    'user_id' => $user->id,
                                    'tipo_accion' => \App\Enums\SolicitudPermisoTipoAccion::CAMBIAR_DENOMINACION_LICENCIA,
                                    'estado' => \App\Enums\SolicitudPermisoEstado::PENDIENTE,
                                    'observacion' => $data['observacion'],
                                ]);
                                ...
                                Notification::make()
                                    ->title('Solicitud Enviada')
                                    ->body('Su solicitud de cambio de denominación ha sido registrada y está pendiente de aprobación.')
                                    ->success()
                                    ->send();
                                ...
                                $recipients = \App\Models\User::permission('view::solicitud_permisos')
                                    ->permission('edit::solicitud_permisos')->get();
                                Notification::make()
                                    ->title('Nueva Solicitud de Permiso para Cambiar Denominación Licencia')
                                    ->body("El usuario " . auth()->user()->name . " ha solicitado un permiso.")
                                    ->icon('heroicon-o-document-text')
                                    ->color('warning')
                                    // AQUÍ EL BOTÓN
