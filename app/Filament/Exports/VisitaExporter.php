    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('sistema')
                ->label('Sistema')
                ->enabledByDefault(false),
            ExportColumn::make('usuing')
                ->label('Registró Ingreso (Nombres)')
                ->formatStateUsing(function ($record) {

                    $nombreUsuario = $record?->userIngreso?->trabajador?->nombres;
                    $apellidoUsuario = $record?->userIngreso?->trabajador?->apellidos;

                    return "{$nombreUsuario} {$apellidoUsuario}";
                })
                ->enabledByDefault(false),
            ExportColumn::make('userIngreso.email')
                ->label('Registró Ingreso (Email)')
                ->enabledByDefault(false),

            ExportColumn::make('ususal')
                ->label('Registró Salida (Nombres)')
                ->formatStateUsing(function ($record) {

                    $nombreUsuario = $record?->userSalida?->trabajador?->nombres;
                    $apellidoUsuario = $record?->userSalida?->trabajador?->apellidos;

                    return "{$nombreUsuario} {$apellidoUsuario}";
                })
                ->enabledByDefault(false),
            ExportColumn::make('userSalida.email')
                ->label('Registró Salida (Email)')
                ->enabledByDefault(false),
        ];
    }
